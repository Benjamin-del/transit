...
export default function Home(data) {
  ...
  async function putRoute(ctnstring, ele) {
    ...
    if (ctnstring) {
      var shpAttr = ftm_ele.getAttribute("shape")
      console.log(shpAttr)
      if (!shpAttr) {
        ...
        var shpAttr = ctnResponse.results[0].shape
        ftm_ele.setAttribute("shape", shpAttr)
      } else {
        ftm_ele.setAttribute("shape", shpAttr)
...
export async function getServerSideProps(context) {
  ...
  const request = await fetch(url + "/api/dynamic/realtime?stop=" + context.params.code)
